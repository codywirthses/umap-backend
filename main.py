...
async def molecule_details(molecule: str):
    ...
    query = f'''
        SELECT SMILES, 
            ROUND(HOMO_EV, 2) as HOMO,
            ROUND(LUMO_EV, 2) as LUMO,
            ROUND(ESP_MAX_EV, 2) as ESP_MAX,
            ROUND(ESP_MIN_EV, 2) as ESP_MIN,
            FUNCTIONAL_GROUPS,
            ROUND(PREDICTED_MP, 2) as PREDICTED_MP,
            ROUND(PREDICTED_BP, 2) as PREDICTED_BP
        FROM UMAP_DATA.PUBLIC.UMAP_1M
        WHERE SMILES = '{smiles["smiles"]}'
        LIMIT 1
    '''
    ...
    try:
        ...
        image_data = smiles_to_image(row['SMILES'])
        if image_data:
